...
BCDecoderPlayer::~BCDecoderPlayer()
{
    ...
    while(!m_mapVideoPlayer.isEmpty())
        delete m_mapVideoPlayer.take(m_mapVideoPlayer.firstKey());
}


bool BCDecoderPlayer::addVideoPlayer(int id, const QString &url)
{
    if (m_mapVideoPlayer.contains(id))
        return false;

    VideoPlayer *pVideoPlayer = new VideoPlayer(id, url, this);
    connect(pVideoPlayer,SIGNAL(sig_GetOneFrame(QImage)),this,SLOT(SetImageSlots(QImage)), Qt::QueuedConnection);

    m_mapVideoPlayer.insert(id, pVideoPlayer);
...
void BCDecoderPlayer::play(int id)
{
    if (m_nCurrentId == id)
        return;
    ...
    VideoPlayer *pVideoPlayer = m_mapVideoPlayer.value(id);

    pVideoPlayer->startDec();

#if 0
    if (pVideoPlayer->refCount() == 1) {
        connect(pVideoPlayer,SIGNAL(sig_GetOneFrame(QImage)),this,SLOT(SetImageSlots(QImage)), Qt::QueuedConnection);
    }
#endif
...
void BCDecoderPlayer::stop()
{
    ...
    if (!m_mapVideoPlayer.contains(m_nCurrentId))
        return;

    VideoPlayer *pVideoPlayer = m_mapVideoPlayer.value(m_nCurrentId);

    pVideoPlayer->stopDec();

#if 0
    if (pVideoPlayer->refCount() == 0) {
        disconnect(pVideoPlayer,SIGNAL(sig_GetOneFrame(QImage)),this,SLOT(SetImageSlots(QImage)));
    }
#endif
...
void VideoPlayer::startDec()
{
    ...
    qCDebug(logPlayer) << "v5" << m_id << m_strPath << m_nRefCount << isRunning();
