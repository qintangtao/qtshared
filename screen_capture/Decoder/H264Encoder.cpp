...
/*// m_pEncoder = new H264Encoder();
    //m_pEncoder->Init(desktopPixmap.width(), desktopPixmap.height(), fps, 30, 2048, 1, 20, 0);
    //m_pEncoder->Init(1920,1080, fps, 5, 1000, 1, 20, 0);
*/
//品质
...
int H264Encoder::Init(int width,int height,int fps,int keyframe,int bitrate,int level,int qp,int nUseQP)
{
    ...
    m_x264_param.b_annexb=1; //每帧前添加0 0 0 1
    m_x264_param.b_repeat_headers=1;//I帧前添加sps pps
...
void H264Encoder::GetSPSAndPPS(unsigned char*sps,long&spslen,unsigned char*pps,long&ppslen)
{
    if (m_hx264!=NULL)
    {
        ...
        spslen=tnal[0].i_payload;
        memcpy(sps,tnal[0].p_payload,spslen);
        ppslen=tnal[1].i_payload;
        memcpy(pps,tnal[1].p_payload,ppslen);
    }else
    {
        spslen=0;
        ppslen=0;
