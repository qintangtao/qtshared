...
namespace Log4Qt
{
                                                               ...
                                                               const QString &rName)
    {
        ...
        exclusions << QLatin1String("layout") << QLatin1String("filter") ;
    ...
    LogObjectPtr<Filter> PropertyConfigurator::parseFilter(const Properties &rProperties,
                                     const QString &rAppenderKey,
                                     int index)
    {

        Q_ASSERT_X(!rAppenderKey.isEmpty(), "PropertyConfigurator::parseFilter()", "rAppenderKey must not be empty.");
        ...
        const QLatin1String layout_suffix(".filter");

        logger()->debug("Parsing filter for appender named '%1'", rAppenderKey);

        QString key = rAppenderKey + layout_suffix + QString(".F%1").arg(index);
        QString value = OptionConverter::findAndSubst(rProperties, key);
        if (value.isNull())
        {
#if 0
            LogError e = LOG4QT_ERROR(QT_TR_NOOP("Missing filter definition for appender '%1'"),
                                      CONFIGURATOR_MISSING_LAYOUT_ERROR,
                                      "Log4Qt::PropertyConfigurator");
            e << rAppenderKey;
            logger()->error(e);
#endif
            logger()->debug("Missing '%1' filter definition for appende '%2'", key, rAppenderKey);
            return 0;
        }
        LogObjectPtr<Filter> p_filter = Factory::createFilter(value);
        if (!p_filter)
        {
            LogError e = LOG4QT_ERROR(QT_TR_NOOP("Unable to create filter of class '%1' requested by appender '%2'"),
                                      CONFIGURATOR_UNKNOWN_FILTER_CLASS_ERROR,
                                      "Log4Qt::PropertyConfigurator");
            e << value << rAppenderKey;
            logger()->error(e);
            return 0;
        ...
        QStringList exclusions;
        setProperties(rProperties, key + QLatin1String("."), QStringList(), p_filter);
