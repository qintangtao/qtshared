...
namespace Log4Qt
{
    ...
    void LogManager::qtMessageHandler(QtMsgType type, const QMessageLogContext &context, const QString &message)
    {
        ...
        QString java_name(context.category);
        if (!java_name.isEmpty())
        {
            ...
            QString parent_name = cpp_name;
            do {
                if (instance()->exists(parent_name))
                    break;

                int index = cpp_name.lastIndexOf(name_separator);
                if (index >=0)
                    parent_name = cpp_name.left(index);
                else
                    break;
            } while(true);
            ...
            if (instance()->exists(parent_name))
            {
                Logger *logger = instance()->logger(parent_name);
                if (instance()->rootLogger() != logger)
                    logger->log(level, message, context);
                else
                    instance()->qtLogger()->log(level, message, context);
            }
            else
            {
                instance()->qtLogger()->log(level, message, context);
            ...
            {
                instance()->logger(cpp_name)->log(level, message, context);
            }
            else
            {
                instance()->qtLogger()->log(level, message, context);
